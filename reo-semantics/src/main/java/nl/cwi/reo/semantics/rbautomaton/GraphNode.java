...
public class GraphNode {
	...
	private List<GraphNode> exclusiveNodes = new ArrayList<>();
	...
	public List<GraphNode> getExcNodes() {
		return exclusiveNodes;
	}

	public void addExcNodes(List<GraphNode> n) {
		exclusiveNodes.addAll(n);
	}
	...
	public void removeParents(List<GraphNode> n) {
		parents.removeAll(n);
	}
	public void removeChilds(List<GraphNode> n) {
		childs.removeAll(n);
	}
	...
	/**
	 * Return true if the current node has an exclusive edge with the other node
	 * 
	 * @param n
	 * @return
	 */
	public boolean hasExclusiveEdge(GraphNode n) {
		Rule x = n.getRule();
		boolean hasExclusiveEdge = false;
		for (Port p : rule.getSync().keySet()) {
			if (rule.getSync().get(p)) {
				if (x.getSync().get(p)!=null && !x.getSync().get(p)) {
					return true;

...
public final class Interpreter<T extends Semantics<T>> {
	...
	public List<T> getProgram(String mainfile) {
		...
		Instance instance = evaluatePrograms(programs);
		
		System.out.println("Interpreter.java: line 59");
		...
		for (Map.Entry<Semantics<?>, Map<String, Port>> atom : instance.getAtomics().entrySet()) {			
	 ...
	 */
	private Instance evaluatePrograms(Stack<SourceFile> programs) {
		...
		return ((ComponentValue)pexpr).getInstance();
	}

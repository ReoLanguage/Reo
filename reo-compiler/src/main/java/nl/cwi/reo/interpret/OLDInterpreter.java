 ...
 */
public class OLDInterpreter extends ListenerBase {
	 ...
	 * Abstract components
	 */
	private ParseTreeProperty<Atom> components = new ParseTreeProperty<Atom>();
	 ...
	 */
	private ParseTreeProperty<Signature> interfaces = new ParseTreeProperty<Signature>();
	...
	@Override public void exitFile(@NotNull ReoParser.FileContext ctx) { 
		try {
			...
			for (ReoParser.CompContext comp_ctx : ctx.comp()) {
				Atom C = components.get(comp_ctx);
	...
	@Override public void exitDefnComposed(@NotNull ReoParser.DefnComposedContext ctx) {
		...
		if (ctx.params() != null)
			for (TerminalNode id : ctx.params().ID())
				params.add(id.getText());
		Signature intface = interfaces.get(ctx.nodeset());
		Set<Atom> comps = new HashSet<Atom>();
		for (ReoParser.CompContext comp_ctx : ctx.comp())
	...
	@Override public void exitNodesName(@NotNull ReoParser.NodesNameContext ctx) { 
		interfaces.put(ctx, new Signature(ctx.ID().getText()));
	}
	...
	@Override public void exitNodeset(@NotNull ReoParser.NodesetContext ctx) { 
		Signature intface = new Signature();
		for (ReoParser.NodesContext node_ctx : ctx.nodes())
			intface = Signature.join(intface, interfaces.get(node_ctx));
	...
	@Override public void exitNodesIndex(@NotNull ReoParser.NodesIndexContext ctx) {
		...
		interfaces.put(ctx, new Signature(node, expr));
	}
	...
	@Override public void exitNodesRange(@NotNull ReoParser.NodesRangeContext ctx) {
		...
		interfaces.put(ctx, new Signature(node, e1, e2)); 
	}

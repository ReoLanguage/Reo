...
import nl.cwi.reo.interpret.TreoParser.ProgContext;
...
import nl.cwi.reo.interpret.TreoParser.Stmt_definitionContext;
...
//		includes.add(new File(filename));
	...
	@Override
	public void enterStmt_definition(Stmt_definitionContext ctx) { }

	@Override
	public void exitStmt_definition(Stmt_definitionContext ctx) {
		progs.put(ctx, new ProgramDefinition(defns.get(ctx.defn())));
	}
	...
	public void exitDefn_definition(Defn_definitionContext ctx) {
		defns.put(ctx, new DefinitionEquation(variables.get(ctx.var()), cexprs.get(ctx.cexpr())));
	}
...
//		values.put(ctx, bexprs.get(ctx.bexpr()));
	...
	public void exitCexpr_composite(Cexpr_compositeContext ctx) {
		cexprs.put(ctx, new ComponentComposite(signatures.get(ctx.sign()), progs.get(ctx.prog())));		
	}

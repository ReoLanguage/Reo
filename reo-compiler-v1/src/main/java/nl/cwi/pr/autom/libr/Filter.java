...
public class Filter extends UserDefinedInitializer {
	...
	public void initialize(UserDefinedAutomaton automaton) {
		...
		constraint = automaton.newConstraint(Arrays.asList(automaton
				.newLiteral(true, extralogicals[0],
						Arrays.asList(automaton.newTerm(inputPorts[0]))),
				automaton.newLiteral(true, automaton.newTerm(inputPorts[0]),
		...
		constraint = automaton.newConstraint(Arrays.asList(automaton
				.newLiteral(false, extralogicals[0],
						Arrays.asList(automaton.newTerm(inputPorts[0])))));
		automaton.addOrKeepTransition(state, state, ports, constraint);
	}
}

...
/**
 * Implements a bipartite directed graph whose edges are labeled by two
 * integers.
 */
public class GameGraph {
	 ...
	 * 
	 * @param v0
	 *            initial vertex of the game
	 * @param owner
	 *            player that owns the initial vertex
	 */
	 ...
	 *            work automaton
	 * @param port
	 *            port where throughput is measured
	 */
		...
		while (!L.isEmpty()) {
			...
			if (a.owner == 0) { // if a = (p, q, N, d)
				...
				for (long i = 0; i < Math.pow(2, A.getJobs().size()); ++i) {
					...
					// Construct subset s of J using the binary representation
					// of i
				...
				for (Transition tau : A.getTransitions(a.q)) {
					...
					if (t >= 0) {
						...
						boolean fires = true;
						for (String x : A.getJobs())
							if (p.get(x) > tau.getJobConstraint().getW().get(x)
									|| (tau.getJobConstraint().getR().contains(x)
											&& p.get(x) < tau.getJobConstraint().getW().get(x)))
								fires = false;
						...
						if (fires) {
							for (String x : tau.getJobConstraint().getR())
								p.put(x, 0);
				...
				// If no edge has been added to vertex a, then add a waiting
				// move
				if (!hasEnabled) {
	 ...
	 *            transition
	 * @param p
	 *            current progress of jobs
	 * @param s
	 *            set of scheduled/running jobs
	...
	private static int ttfc(Transition tau, Map<String, Integer> p, Set<String> s) {
		...
		// If no jobs are scheduled then t = 0, if p satisfies w, and t = -1
		// otherwise
		if (s.isEmpty()) {
	 ...
	 *            time of the edge
	 * @param i
	 *            owner of the first vertex
	 * @param j
	 *            owner of the second vertex
	 */
			...
			for (Vertex v : G.vert.values()) {
				for (Edge e : G.adjc.get(v)) {
					if (v.equals(e.source)) {
						String optimal = (G.vert0.contains(v) && G.adjc0.get(v).contains(e)) ? "style=solid"
								: "style=dotted";
	...
	/**
	 * Computes minimal credit function on the energy game associated with the
	 * ratio game with winning condition <code>value &gt;= v</code>, for some
	 * fraction v = p/q.
	 * 
	 * @param p
	 *            numerator of value v
	 * @param q
	 *            denominator of value
	 * @return the minimal credit function on the energy game that contains only
	 *         optimal vertices and edges with weights w'(e) = q * w(e) - p *
	 *         t(e), for every edge e
	 */
	public HashMap<Vertex, Double> minimalCredit(int p, int q) {
		...
		for (Vertex v : vert0) {
			...
			f.put(v, 0.0);
			count.put(v, cnt);
			if ((v.owner == 0 && cnt == 0) || (v.owner == 1 && ic))
				L.add(v);
			M += Collections.max(w); // is this still correct in throughput
										// games?
		}
	...
	/**
	 * Removes all vertices from the game graph that are not used by some
	 * optimal strategy.
	 */
	public void synthesize() {

		// This code assumes that there the minimal weight is <= 0 and the
		// maximal weight is >= 0.
		int V0 = V;
		int pmin = 0;
		int qmin = 1;
		...
		while (V0 * (V0 - 1) * (pmax * qmin - pmin * qmax) >= qmin * qmax) {
			...
			// System.out.println("p0/q0 = " + pmin + "/" + qmin + "; " + "p/q =
			// " + p + "/" + q + "; " + "p1/q1 = " + pmax + "/" + qmax + "; ");
			// System.out.println("f = " + f);

			// the code does not handle atomic cycles, i.e., cycles C with \sum
			// t(C) = 0

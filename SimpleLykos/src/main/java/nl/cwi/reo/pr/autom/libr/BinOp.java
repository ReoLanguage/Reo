...
public class BinOp extends UserDefinedInitializer {
	...
	public void initialize(UserDefinedAutomaton automaton) {
		...
		ports = Arrays.asList(inputPorts[0], inputPorts[1], outputPorts[0]);
		constraint = automaton.newConstraint(Arrays.asList(automaton
				.newLiteral(true, automaton.newTerm(extralogicals[0], Arrays
						.asList(automaton.newTerm(inputPorts[0]),
								automaton.newTerm(inputPorts[1]))), automaton
						.newTerm(outputPorts[0]))));
